...
test("dialogue popup has independent On/Off and mix, quiet failures and recovery", async () => {
  const h = createHarness();
  h.createFrame(1);
  ...
  assert.equal(e.dialogueStatus, undefined); assert.equal(e.dialogueError, undefined);
